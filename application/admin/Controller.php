...
class Controller
{
    ...
    /****
     *  压缩文件夹
     *  @param string     $folder            要压缩的文件夹名称
     *  @param string     $zipFile           压缩类
     *  @param string     $subfolder         子文件夹路径
    */
    protected function folderToZip($folder, &$zipFile, $subfolder = null) 
    {
        if ($zipFile == null) {
            // 没有资源
            return false;
     ...
     * @param string     $contentType     文件的类型
    */
    protected function downZip($name, $contentType="")
    {
        if ($contentType) {
    ...
    */
     ...
     */
    protected function log($module, $type, $id, $data, $content = '')
    {
        $module_main = $module[0];
        if(count($module) == 1){
            $module_son = $module[0];
        }else{
            $module_son = '';
            ...
            $module_son = rtrim($module_son, '_');
        }
        Db::name('log')->insert(array('admin' => Session::get('user_name'), 'admin_id' => Session::get('auth_id'), 'module_main' => $module_main, 'module_son' => $module_son, 'type' => $type, 'record_id' => $id, 'data' => json_encode($data), 'ip' => $this->request->ip(), 'content' => $content, 'time' => time()));
    }
}