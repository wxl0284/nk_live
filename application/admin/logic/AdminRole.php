...
class AdminRole
{
    ...
    public function getAccessTree($role_id)
    {
        ...
        $group_ids = Db::name('admin_group')->where($where_node)->order('sort asc')->column('id');
        ...
        $node = Db::name("AdminNode")->where("status=1 AND isdelete=0")
                ->where(function ($query) use ($group_ids) {
                    $query->where(['group_id'=>['in',$group_ids]])->whereor(['group_id'=>0]);
                })
                ->field("id,pid,group_id,name,title,level,type")
