...
class NodeMap
{
    ...
    public function load($module, $requestMethod = self::METHOD_ALL, $filter = [])
    {
        ...
        $adminNode = Db::name('admin_node')->where(['level'=>2])->column('title','name');
        // 读取指定模块的类
        ...
        foreach ($classes as $class) {
            ...
            foreach ($class['method'] as $method) {
                if (isset($adminNode[$class['class_name']]) && (!isset($maps[$controller]) || !isset($maps[$controller][$method['name']]))) {
                    $insertMap[] = [
                        'module'     => $module,
                        'controller' => $controller,
                        'action'     => $method['name'],
                        'method'     => $requestMethod,
                        'comment'    => ($adminNode[$class['class_name']] ?? '系统') . '下， ' . $method['title'],
                    ];
                }
            }
