        ...
        foreach ($menu as $k => $m) {
            ...
            switch ($m) {
                ...
                case 'sedit':
                    ...
                    $parseStr .= ' <a title="' . $title . '" href="javascript:;" onclick="layer_open(\'' . $title . '\',\'<?php echo \think\Url::build(\'' . $url . '\', [' . $param . ']); ?>\')" style="text-decoration:none" class="ml-5"><i class="Hui-iconfont">编辑</i></a>';
                    break;
                case 'show':
                case 'sshow':
                ...
                case 'import': // zhitian add 生成excel
                    ...
                    $parseStr .= '<a class="btn btn-primary radius mr-5" href="javascript:;" onclick="layer_open(\'' . $title . '\',\'<?php echo \think\Url::build(\'' . $url . '\', [' . $param . ']); ?>\')"><i class="Hui-iconfont">&#xe645;</i> ' . $title . '</a>';
                    break;
                case 'sub-add': //添加页面里的增加
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '添加';
                    list($url, $param) = $this->parseUrl($url);
                    $parseStr .= '<a class="btn btn-primary radius mr-5" href="javascript:;" onclick="subAdd()"><i class="Hui-iconfont">&#xe600;</i> ' . $title . '</a>';
                    break;
                case 'sub-add-equip': //添加页面里的增加
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '添加';
                    list($url, $param) = $this->parseUrl($url);
                    $parseStr .= '<a class="btn btn-primary radius mr-5" href="javascript:;" onclick="subAddEquip()"><i class="Hui-iconfont">&#xe600;</i> ' . $title . '</a>';
                    break;
                case 'sub-delete':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '删除';
                    list($url, $param) = $this->parseUrl($url);
                    $parseStr .= '<a href="javascript:;" onclick="subDelete()" class="btn btn-danger radius mr-5"><i class="Hui-iconfont">&#xe6e2;</i> ' . $title . '</a>';
                    break;
                case 'sub-delete-equip':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '删除';
                    list($url, $param) = $this->parseUrl($url);
                    $parseStr .= '<a href="javascript:;" onclick="subDeleteEquip()" class="btn btn-danger radius mr-5"><i class="Hui-iconfont">&#xe6e2;</i> ' . $title . '</a>';
                    break;
                case 'sub-up':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '上移';
                    list($url, $param) = $this->parseUrl($url);
                    $parseStr .= '<a href="javascript:;" onclick="moveUpward()" class="btn btn-primary radius mr-5"><i class="Hui-iconfont">&#xe679;</i> ' . $title . '</a>';
                    break;
                case 'sub-down':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '下移';
                    list($url, $param) = $this->parseUrl($url);
                    $parseStr .= '<a href="javascript:;" onclick="moveDown()" class="btn btn-success radius mr-5"><i class="Hui-iconfont">&#xe674;</i> ' . $title . '</a>';
                    break;
                default:
                    // 默认为小菜单
