...
class Subject extends Wx
{
    ...
    public function evaluate_list(){
        $get = $this->request->get();
        $subject_evaluate = Db::name("subject_evaluate")->where(['subject_id'=>$get['id']])->limit($get['start'],$get['limit'])->select();
        if($subject_evaluate){
            foreach($subject_evaluate as $key => $val){
                ...
                $subject_evaluate[$key]['created'] = round((time()- $val['create_time'])/3600/24);
    ...
    public function reply_list(){
        $get = $this->request->get();
        $subject_reply = Db::name("subject_reply")->where(['evaluate_id'=>$get['pid']])->limit($get['start'],$get['limit'])->select();
        if($subject_reply){
            foreach($subject_reply as $key => $val){
                $user = Db::name("user")->where(['id'=>$val['user_id']])->field("name,img")->find();
                $subject_reply[$key]['name'] = $user['name'];
                $subject_reply[$key]['img'] = $user['img'];
                $subject_reply[$key]['created'] = round((time()- $val['create_time'])/3600/24);
            }
        }
