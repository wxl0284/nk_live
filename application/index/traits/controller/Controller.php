    ...
    public function excel_import($file,$step){
       ...
       //  dump($excelHeader);die;
        ...
        // array(10) {
            ...
            // }
        if($step == 1){
            $header = ['A' => 'equip_name', 'B' => 'name_unit', 'C' => 'equip_model', 'D' => 'manudacture', 'E' => 'equip_factory_num'];
            ...
            $header = ['A' => 'number', 'B' => 'matter', 'C' => 'purchase_name', 'D' => 'specification', 'E' => 'unit_measure', 'F' => 'quantity_number', 'G' => 'amount_number', 'H' => 'route_number', 'I' => 'supply_commodity', 'J' => 'feed_basis','K' => 'ware_spare','L' => 'number_access','M' => 'quantity_standard', 'N' => 'remarks'];
            $result = \Excel::parse($file, $header, 4,1, function ($data) {
                ...
                foreach ($data as &$v) {
                    ...
                    if($v['purchase_name']){
                        ...
                        $today = date("Ymd",time());
                        $v['request_no'] = $today;
                        $rand_str = getRandStr(4);
                        $v['purchase_num'] = 'CGD'.$today.$rand_str.substr(time(),-2);
                        $v['create_time'] = time();
                        $v['applicant'] = $_SESSION['think']['userid'];
                        //$v['supply_commodity'] = strtotime(\Excel::excelTime($v['supply_commodity'], $time=false));
                        $v['supply_commodity'] = strtotime($v['supply_commodity']);
