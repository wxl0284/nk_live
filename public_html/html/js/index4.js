...
X.sub("init", function() {
    ...
    function onImgContent() {
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            for (var j = 0; j < result[i].length; j++) {
                ...
                res += '<div class="subs-icon"><i class="iconfont">' + '' + '</i></div>';
                res += '<div class="subs-list">';
                res += '<h1><a href="details.html" onclick="X.pub(\'getMenuSubject\',\'' + item.num + '\')">' + item.title + '</a></h1>';
                res += '<span>' + (LANG == "EN" ? "" : "对应专业:") + '</span>';
                ...
                for (var f = 0; f < size; f++) {
                    ...
                    res += '<a href="details.html" onclick="X.pub(\'getMenuSubject\',\'' + child.num + '\')">' + child.title + '</a>';
                    if ((f + 1) !== size) {
                        res += '<span>/</span>';
                    } else {
                        if (isMore) {
    ...
    function loadNew1() {
        X.get('/json/infos?sortby=id&reverse=true&status=1&type=1&del=0&limit=2', function(respText) {
            ...
            var div = '';
            if (resp.meta.size == '0') {
                div += '<div class="noitems">' + (LANG == "EN" ? EN.index.noNotices : "暂无通知公告") + '</div>';
            } else {
                for (var i = 0, size = resp.meta.size; i < size; i++) {
                    var item = resp.data[i];
                    var len = 160;
                    if (item.title.length > 22) {
                        len = 100;
                    }
                    item.date = item.date || moment(item.created).format("YYYY-MM-DD");
                    var dates = item.date.split("-");
                    div += '<li>';
                    // div += '<a href="/infodetails?id=' + item.id + '">';
    ...
    function loadNew2() {
        X.get('/json/infos?sortby=id&reverse=true&status=1&type=4&del=0&limit=2', function(respText) {
            var resp = JSON.parse(respText);
            resp.meta = resp.meta || {
                size: '0',
                total: '0'
            ...
            var div = '';
            if (resp.meta.size == '0') {
                div += '<div class="noitems">' + (LANG == "EN" ? EN.index.noNews : "暂无新闻资讯") + '</div>';
            } else {
                for (var i = 0, size = resp.meta.size; i < size; i++) {
                    var item = resp.data[i];
                    var len = 160;
