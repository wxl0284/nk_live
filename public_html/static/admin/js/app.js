 ...
 * @param url
 */
function open_window_xin(title, url, nav) {
    //解决在非iframe页里打开不了页面的问题
    if (window.parent.frames.length == 0) {
        window.open(url);
        return false;
    ...
    if (!bStop) {
        ...
        iframeBox.hide();
        var ts = get_random();
        iframe_box.append('<div class="show_iframe"><div class="loading"></div><iframe frameborder="0" src=' + url + ' id=' + ts + ' name="' + ts + '"></iframe></div>');
        var showBox = iframe_box.find('.show_iframe:visible');
        showBox.find('iframe').attr("src", url).load(function () {
            showBox.find('.loading').hide();
            if (nav != '') {
