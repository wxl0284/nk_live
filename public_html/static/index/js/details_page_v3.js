...
X.sub("init", function() {
    ...
    function onGetContent(respText) {
        ...
        if(item.collect_s == 0){
            ...
            res += '<button type="button" class="btn btn-white btn-opacity" id="collectionBtn"><img src="/static/index/pic/collection.png" style="vertical-align:middle;"><span style="vertical-align:middle;">&nbsp;已收藏</span></button>';
            //res += '<button type="button" class="btn btn-white btn-opacity" id="collectionBtn" style="background-image: url(/static/index/pic/collection2.png);"><i class="iconfont"></i></button>';
        }
        if(item.like_s == 0){
            res += '<button type="button" class="btn btn-white btn-opacity" id="likeBtn"><img src="/static/index/pic/clickgood.png" style="vertical-align:middle;"><span style="vertical-align:middle;">&nbsp;点赞</span></button>';
			//res += '<button type="button" class="btn btn-white btn-opacity" id="likeBtn" Style="background-image: url(/static/index/pic/clickgood.png);"><i class="iconfont"></i></button>';
        }else{
            res += '<button type="button" class="btn btn-white btn-opacity" id="likeBtn"><img src="/static/index/pic/clickgood.png" style="vertical-align:middle;"><span style="vertical-align:middle;">&nbsp;已点赞</span></button>';
			//res += '<button type="button" class="btn btn-white btn-opacity" id="likeBtn" Style="background-image: url(/static/index/pic/clickgood.png);"><i class="iconfont"></i></button>';
        }
        ...
        function gotoConfirm() {
            ...
            var item = {};
                item.title = "提示";
                item.msg = '<p class="goLink-tip" style="text-align:center;">是否要做实验。</p><p class="goLink-link" style="text-align:center;"><a href="javascript:;" id="confirmSubject">确认</a></p>';
                item.okText = "取消";
            item.noCancel = true;
        ...
        $(document).on("click","#confirmSubject",function(){//与国家平台对接前的流程
            X.post("/index/subject/examine_url", {
                ...
                resp = JSON.parse(resp);
               // resp = resp.result;
				//resp.result = 'http://www.baidu.com|http://www.baidu.com';
                if (resp.code == '200') {
					//在此处对返回的result进行判断，如果是2个url则在弹框显示'专家实验入口'和'学生实验入口'
					let url = [];
					if ( resp.result.indexOf('|') > -1 )//是两个网址
					{
						url = resp.result.split('|');
		...
		function gotoConfirm_1(url) {

            var item = {};
                item.title = "提示";
                item.msg = '<p class="goLink-link" style="text-align:center;"><a href="' + url[0] + '">专家实验入口</a> <br> <a href="' + url[1] + '">人-人对抗版实验</a></p>';
                item.okText = "取消";
            item.noCancel = true;
            X.pub("showDialog", item);
        }
        ...
        $("#likeBtn").unbind().on("click", function() {
            ...
            X.post("/index/subject/like_add", item, function(resp) {
                ...
                if(resp.code == 100){
                   ...
                   if(resp.msg=='点赞成功！'||resp.msg=='您已经点赞！'){
                        /* $("#likeBtn").addClass("likeBtn2")
                        $("#likeBtn").css("background-image","url(/static/index/pic/clickgood2.png)") */
						 $("#likeBtn").find('span').html('已点赞');
                    }else{
                        /* $("#likeBtn").removeClass("likeBtn2") 
                        $("#likeBtn").css("background-image","url(/static/index/pic/clickgood.png)")*/
                    }
                    X.dialog(resp.msg);
                    /* loadContent();
        ...
        $("#collectionBtn").off('click').on("click", function() {
            ...
            X.post("/index/subject/collection_add", item, function(resp) {
                ...
                if(resp.code == 100){
                    ...
                    if(resp.msg=='收藏成功！'){
                        /* $("#collectionBtn").addClass("collectionBtn2") 
                        $("#collectionBtn").css("background-image","url('/static/index/pic/collection2.png')")*/
						$("#collectionBtn").html('<img src="/static/index/pic/collection.png" style="vertical-align:middle;"><span style="vertical-align:middle;">&nbsp;取消收藏</span>');
						
                    }else{
                        /* $("#collectionBtn").removeClass("collectionBtn2") 
                        $("#collectionBtn").css("background-image","url('/static/index/pic/collection.png')")*/
						$("#collectionBtn").html('<img src="/static/index/pic/collection.png" style="vertical-align:middle;"><span style="vertical-align:middle;">&nbsp;收藏</span>');
                    }
                    X.dialog(resp.msg);
                }
            }, function(code) {
    ...
    function initInfoScore() {
        var res = '<div class="starbox left" style="margin-top: -4px;">';
        for (var i = 0; i < 5; i++) {
            if (i < project.score) {
                res += '<span class="star yes"></span>';
            } else {
                res += '<span class="star no" style="position: relative;top: -1px;font-size: 20px;">☆</span>';
            }
        }
        res += '</div>';
    ...
    function setCuttle() {
        ...
        $(".showMore").off('click').on('click', function() {
            if ($(this).hasClass("active")) {
                ...
                $(this).attr("title", "收起更多").find("span").fadeOut('fast').html('<span>收起更多<i class="iconfont">&nbsp;<img src="/static/index/pic/jjtt.png" width="10px"></img></span>').fadeIn('fast');
            }
        });
    }
    ...
    function onLoadDiscuss(respText) {
        ...
        if (res.meta.total != '0') {
            ...
            for (var i = 0; i < size; i++) {
                ...
                div += '<div class="left">';
                div += '<a href="javascript:void(0);"><img src="' + ("/"+item.img) + '"  onerror="this.src=\'' + DEFAULT_AVATAR + '\'" alt="' + item.name + '" width="50" height="50" /></a>';
                div += '</div>';
                div += '<div class="body">';
                div += '<h4>' + (item.name || "未知用户") + '<small><span>' + (item.created) + '</span>发表</small></h4>';
                div += '<p>' + (item.content || "...") + '</p>';
                div += '<div class="append">';
                div += '<a href="javascript:void(0);" data-reply="' + item.id + '" data-count="' + (item.reply_count || 0) + '"><i class="iconfont talk"><img src="/static/index/pic/huifu.jpg" style="width:18px;height:18px;margin-bottom: -4px;" /></i>回复(' + (item.reply_count || 0) + ')</a>';
                div += '</div>';
                div += '</div>';
                div += '</li>';
            }
        ...
        $('[data-reply]').click(function(e) {
            ...
            if ($('#replyBox-' + id).length == 0) {
                ...
                $('#replyBox-' + id).remove();
                $this.html('<i class="iconfont talk"><img src="/static/index/pic/huifu.jpg" style="width:18px;height:18px;margin-bottom: -4px;" /></i> 回复(' + count + ')');
            }
        });
    }
    ...
    function onLoadComments(respText, pid) {
        ...
        $('[data-reply=' + pid + ']').html('<i class="iconfont talk"><img src="/static/index/pic/jjtt.png" style="width:10px;height:15px;margin-bottom: -2px;" /></i>收起回复');
        X.pub('closeDialog');
        ...
        if (isTure) { // 已评分
            ...
            for (var i = 0; i < 5; i++) {
                if (i < project.score) {
                    ...
                    div2 += '<span class="star no" style="position: relative;top: -5px;">☆</span>';
                }
            }
            div2 += '</div>';
