...
X.sub("init", function() {
    ...
    function loadContent() {
        X.get('/index/article/detail_up?id=' + vid, onGetContent);
    }
    ...
    function onGetContent(respText) {
        ...
        res += '<div class="info"><span class="source">' + (item.types || "") + '</span><span class=""time>' + (item.create_time) + '</span></div>';
        res += '<div class="article">' + (item.content || "")  + '</div>';
        res += '</div>';
