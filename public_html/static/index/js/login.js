...
X.sub("init", function() {
    ...
    function onLogin(type, cred) {
        ...
        function onGotNonce(respText) {
            ...
            var captcha = (cred.captchaToken) ? "&captchaToken=" + encodeURIComponent(cred.captchaToken) : "";
            X.post("/index/user/login_up?username=" + cred.username + "&password=" + hash + "&nonce=" + resp.nonce + "&cnonce=" + cn + captcha, null, cred.onLoginResponse);
        }
        X.get("/index/user/login_up", onGotNonce);
    }
