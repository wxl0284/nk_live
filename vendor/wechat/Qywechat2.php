...
class Qywechat
{
	...
	const SERVICE_GET_LOGIN 	= '/service/get_login_info?';
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function getCache($cachename){
        ...
        return Cache::get($cachename,false);
		//return false;
	}
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		$authname = 'qywechat_access_token'.$appid.$this->agentid;
	...
	*
	**/
	public function getShareAppMessage($msg_config, $timestamp=0, $appid=''){
	    ...
	    $Message = array(
	      ...
	      "desc"     => $msg_config['desc'],
	      "link"     => $msg_config['link'],
	      "imgUrl"     => "$protocol$_SERVER[HTTP_HOST]".'/'.$msg_config['imgUrl'],
	    );
	    // dump($signPackage);
	    return $Message;
	}
	 ...
	 * }
	 */
	public function _uploadMedia($data, $type){
		if (!$this->access_token && !$this->checkAuth()) return false;
	...
	public function uploadMedia($data, $type){
		if (!$this->access_token && !$this->checkAuth()) return 01;
		$result = $this->http_post(self::API_URL_PREFIX.self::MEDIA_UPLOAD_URL.'access_token='.$this->access_token.'&type='.$type,$data,true);
		var_dump($result);
		if ($result)
		{
	...
	public function getOneRedirect($callback,$state='STATE',$scope='snsapi_base'){
	    $url="https://qy.weixin.qq.com/cgi-bin/loginpage?corp_id=$this->appid&redirect_uri=".urlencode($callback)."&state=$state&usertype=member";
	    return $url;
	}

	public function GetUser($access_token,$code){
    ...
    public function userIdToOpenid(string $userid, int $agentid = null)
    {
		...
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX.'/user/convert_to_openid?'.'access_token='.$this->access_token,self::json_encode($data));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function openidToUserId(string $openid)
    {
		...
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX.'/user/convert_to_userid?'.'access_token='.$this->access_token,self::json_encode($data));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
